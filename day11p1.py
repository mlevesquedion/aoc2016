...
# Pu, Pm, Ru, Sr, Tm
initial_state = tuple([0] + [0, 1] + [2, 2] + [2, 2] + [0, 1] + [0, 0])
want_state = tuple([3] * 11)

def legal(state):
    generators = {state[i] for i in range(1, 10, 2)}
    for generator, microchip in [state[i:i+2] for i in range(1, 10, 2)]:
...
while Q:
    ...
    for state in Q:
        ...
        me = state[0]
        movable = [index+1 for (index, floor) in zip(range(10), state[1:]) if floor == me]
        candidate_states = []
